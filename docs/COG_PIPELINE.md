...
**Scene discovery** uses the Element 84 Earth Search STAC API via `pystac-client` (`src/ember/services/stac.py:147`). A query specifies bbox and date range. Earth Search returns STAC items with asset hrefs pointing to S3 COG paths, sorted by acquisition datetime descending (most recent first) with no cloud-cover filtering applied — for wildfire intelligence, recency and ground-truth coverage outweigh visual clarity. The `_item_to_scene()` function (`src/ember/services/stac.py:121`) maps Earth Search's common-name asset keys (`red`, `nir`, `swir16`) back to canonical Sentinel-2 band IDs (`B04`, `B08`, `B11`).

**Multi-scene stitching** handles bboxes that span MGRS tile boundaries. `search_coverage()` (`src/ember/services/stac.py`) resolves the set of MGRS 100 km grid squares that geographically intersect the requested bbox using the `mgrs` library — this happens locally before any STAC query fires. It then runs a progressive search loop driven by `SENTINEL_SEARCH_WINDOWS` (default `[30, 60, 90]` days), querying Earth Search with a 30-day look-back first; if any pre-computed tile is still uncovered after `pick_best_per_tile()` runs (which selects the most recent scene per tile), it widens to 60 days, then 90 days, merging results across windows so that fresher scenes from narrower windows are preserved for already-covered tiles. The per-window fetch limit scales with the required tile count (`max(20, len(required_tiles) * 2)`) so a 40-tile bbox isn't truncated to the 20 most-recent scenes overall. Tiles with no usable scene across the full 90-day history are detected and named in a warning log; they remain absent from the mosaic and render transparent on the client (Phase 2 `nodata=0` handles this). `read_bands_mosaic()` (`src/ember/services/sentinel_cog.py`) reads each band from each scene at the scene's own native MGRS tile bbox in parallel, then places each tile in a shared canvas at its geographic offset within the union of all tile bboxes. The returned bbox is the union — slightly larger than the requested viewport, which gives clients a panning buffer for free. On overlap, the first-listed tile wins (canvas-equals-zero sentinel preserves earlier fills).
...
mgrs              ─── MGRS 100 km tile resolution from WGS84 lat/lon (bbox→tile-set pre-computation)
```
