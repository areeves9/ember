...
**Multi-scene stitching** handles bboxes that span MGRS tile boundaries. `search_coverage()` (`src/ember/services/stac.py`) runs a progressive search loop driven by `SENTINEL_SEARCH_WINDOWS` (default `[30, 60, 90]` days). It queries Earth Search with a 30-day look-back first; if any tile identified in that window is still uncovered after `pick_best_per_tile()` runs, it widens to 60 days, then 90 days, merging results across windows so that fresher scenes from narrower windows are preserved for already-covered tiles. If a tile has no usable scene within the full 90-day history, it remains absent from the mosaic and renders transparent on the client (Phase 2 `nodata=0` handles this). `read_bands_mosaic()` (`src/ember/services/sentinel_cog.py`) reads each band from each scene in parallel and composites them onto a single canvas — pixels from later scenes fill gaps left by earlier ones.

**Adaptive read method** (ORQ-141 Phase 4): when `read_bands_mosaic()` stitches data from multiple MGRS tiles, it chooses between `Reader.part(bbox)` and `Reader.preview()` on a per-scene basis using a planar-degrees² area ratio. For each scene, if the viewport area (degrees²) is larger than the scene's bounding-box area, the service calls `.preview(max_size=OVERVIEW_MAX_SIZE)` — a downsampled full-scene read from the COG pyramid — rather than a native-resolution crop. The motivation is to avoid saturating S3 with high-resolution range requests at continental zoom levels where the pixel budget (~1200px canvas) would discard most of the fetched data anyway. Conversely, if the viewport is contained within or roughly equal to a single scene (typical tactical/incident view), the existing `.part(bbox)` path is used for full-detail crop. The decision is made per scene, not per request: a continental viewport spanning four MGRS tiles uses `.preview()` for all four; a small viewport whose bbox is entirely inside one scene uses `.part()`. The comparison uses planar degree-area (no geodesic correction) because the ratio is all that matters — geodesic distortion cancels at the comparison step.
